...
//			is = new FileInputStream(f);
	...
	public static void downloadFile(HttpServletResponse response , File file){
		...
		try {
			...
			// attachment弹出对话框，提示用户进行下载保存本地
			// 默认为inline方式
			response.setHeader("Content-Disposition", "attachment;filename="+"userTemp.xls");
	...
	public static void downloadFile(HttpServletResponse response ,String tempPath, HSSFWorkbook workbook){
		...
		try {
			...
			File f = new File(tempPath+"/"+PropertiesUtil.getProperties("excelTempPath")+"/baotList.xls");
			//创建file文件
			f.createNewFile();
			workbook.write(f);
			...
			// attachment弹出对话框，提示用户进行下载保存本地
			// 默认为inline方式
			response.setHeader("Content-Disposition", "attachment;filename="+"baotList.xls");
	...
	public static  String  uploadFile(MultipartFile uploadFile , String fileName , String uploadPath ){
		...
		try {
			...
			closeIO(is, bis, fos, bos,null);
		}
		return uuidName;
	}
	
	
public static void closeIO(InputStream fis , BufferedInputStream bis , OutputStream fos , BufferedOutputStream bos,HSSFWorkbook workbook){
