...
public class JavaMailUtil {
  ...
  public static final  Logger log = Logger.getLogger(JavaMailUtil.class);
    ...
    public static int sendJavaMail(HttpServletRequest request, EmailRecord mail){
        int x = 0;
        try {
        //1.创建 邮件会话
        Session session = createJavaMailSession(mail.getAddresser(),mail.getSendPwd());
        ...
        // 设置发件人
            ...
            // 设置抄送人
            if (null != mail.getCopyrecipients() && !"".equals(mail.getCopyrecipients())){
                message.addRecipients(Message.RecipientType.CC,mail.getCopyrecipients());
            }
            // 设置密送人
            if (null != mail.getSendpeople() && !"".equals(mail.getSendpeople())){
                message.addRecipients(Message.RecipientType.BCC,mail.getSendpeople());
            }
            //设置邮件主题
            message.setSubject(mail.getEmailtheme());
          ...
          //  String realPath = request.getServletContext().getRealPath(ConfigUtil.get("mailAttachPath"));
            String href= "F:\\IdeaProjects\\qyfparent\\qyf-moudules\\md-backstage\\target\\md-backstage\\mailAttachFolder";

            //循环 创建 邮件附件对象
            if (null != mail.getAccessoryname() && !"".equals(mail.getAccessoryname())){
                String[] attachArray = mail.getAccessoryname().split(",");
                for (int i = 0; i < attachArray.length; i++) {
                    MimeBodyPart body = new MimeBodyPart();
                    DataHandler handler = new DataHandler(new FileDataSource(href+"/"+attachArray[i]));
